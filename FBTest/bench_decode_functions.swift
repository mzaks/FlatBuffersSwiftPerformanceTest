...
//  FBTest
//
//  Created by Maxim Zaks on 24.09.16.
//  Copyright © 2016 maxim.zaks. All rights reserved.
//

import Foundation

func fromByteArray<T : Scalar>(buffer : UnsafePointer<UInt8>, _ position : Int) -> T{
    return UnsafePointer<T>(buffer.advancedBy(position)).memory
}

func getPropertyOffset(buffer : UnsafePointer<UInt8>, _ objectOffset : Offset, propertyIndex : Int)->Int {
...
func getOffset(buffer : UnsafePointer<UInt8>, _ objectOffset : Offset, propertyIndex : Int) -> Offset?{
...
func getVectorLength(buffer : UnsafePointer<UInt8>, _ vectorOffset : Offset?) -> Int {
...
func getVectorOffsetElement(buffer : UnsafePointer<UInt8>, _ vectorOffset : Offset, index : Int) -> Offset? {
...
func get<T : Scalar>(buffer : UnsafePointer<UInt8>, _ objectOffset : Offset, propertyIndex : Int, defaultValue : T) -> T{
    let propertyOffset = getPropertyOffset(buffer, objectOffset, propertyIndex: propertyIndex)
    if propertyOffset == 0 {
        return defaultValue
    }
...
func get<T : Scalar>(buffer : UnsafePointer<UInt8>, _ objectOffset : Offset, propertyIndex : Int) -> T?{
    let propertyOffset = getPropertyOffset(buffer, objectOffset, propertyIndex: propertyIndex)
    if propertyOffset == 0 {
        return nil
    }
...
func getStringBuffer(buffer : UnsafePointer<UInt8>, _ stringOffset : Offset?) -> UnsafeBufferPointer<UInt8>? {
...
func getString(buffer : UnsafePointer<UInt8>, _ stringOffset : Offset?) -> String? {
    guard let stringOffset = stringOffset else {
        return nil
    }
    let stringPosition = Int(stringOffset)
